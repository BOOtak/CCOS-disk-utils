...
int ccos_get_dir_contents(ccos_disk_t* disk, ccos_inode_t* dir, uint8_t* data, uint16_t* entry_count, ccos_inode_t*** entries) {
  uint8_t* dir_contents = NULL;
  size_t dir_size = 0;
  if (ccos_read_file(disk, dir, data, &dir_contents, &dir_size) == -1) {
  ...
  *entry_count = dir->desc.dir_count;
  int res = parse_directory_data(disk, data, dir_contents, dir_size, *entry_count, &elements);
  free(dir_contents);
...
int ccos_replace_file(ccos_disk_t* disk, ccos_inode_t* file, const uint8_t* file_data, uint32_t file_size, uint8_t* image_data) {
  ...
  if (get_file_blocks(disk, file, image_data, &block_count, &blocks) != 0) {
  ...
  for (size_t i = 0; i < block_count; ++i) {
    ...
    size_t data_size = 0;
    if (get_block_data(disk, blocks[i], image_data, &start, &data_size) != 0) {
...
char* ccos_get_image_label(ccos_disk_t* disk, uint8_t* data, size_t data_size) {
  ccos_inode_t* root = ccos_get_root_dir(disk, data, data_size);
...
int ccos_set_image_label(ccos_disk_t* disk, uint8_t* data, size_t data_size, const char* label) {
  char newlab[strlen(label) + 1];
  ccos_inode_t* root = ccos_get_root_dir(disk, data, data_size);
  if (strcmp(label, "")) {
    sprintf(newlab, " %s", label);
    return ccos_rename_file(disk, data, data_size, root, newlab, NULL);
  } else {
    return ccos_rename_file(disk, data, data_size, root, "", NULL);
  }
}
...
int ccos_read_file(ccos_disk_t* disk, ccos_inode_t* file, const uint8_t* image_data, uint8_t** file_data, size_t* file_size) {
  size_t blocks_count = 0;
  uint16_t* blocks = NULL;

  if (get_file_blocks(disk, file, image_data, &blocks_count, &blocks) == -1) {
  ...
  *file_data = (uint8_t*)calloc(*file_size, sizeof(uint8_t));
  ...
  for (int i = 0; i < blocks_count; ++i) {
    ...
    size_t data_size = 0;
    if (get_block_data(disk, blocks[i], image_data, &data_start, &data_size) == -1) {
...
int ccos_write_file(ccos_disk_t* disk, ccos_inode_t* file, uint8_t* image_data, size_t image_size,
  ...
  ccos_bitmask_list_t bitmask_list = find_bitmask_blocks(disk, image_data, image_size);
  ...
  if (out_blocks_count > blocks_count) {
    ...
    for (int i = 0; i < (out_blocks_count - blocks_count); ++i) {
      ...
      if (add_block_to_file(disk, file, image_data, &bitmask_list) == CCOS_INVALID_BLOCK) {
    ...
    for (int i = 0; i < (blocks_count - out_blocks_count); ++i) {
      ...
      if (remove_block_from_file(disk, file, image_data, &bitmask_list) == -1) {
  ...
  for (int i = 0; i < blocks_count; ++i) {
    uint8_t* start = NULL;
    size_t data_size = 0;
    get_block_data(disk, blocks[i], image_data, (const uint8_t**)&start, &data_size);
...
// directory
int ccos_copy_file(ccos_disk_t* disk, uint8_t* dest_image, size_t dest_image_size, ccos_inode_t* dest_directory,
                   const uint8_t* src_image, ccos_inode_t* src_file) {
  ccos_bitmask_list_t dest_bitmask_list = find_bitmask_blocks(disk, dest_image, dest_image_size);
  ...
  if ((free_block = get_free_block(disk, &dest_bitmask_list)) == CCOS_INVALID_BLOCK) {
    fprintf(stderr, "Unable to copy file: no space left!\n");
    return -1;
  }

  mark_block(disk, &dest_bitmask_list, free_block, 1);
  ccos_inode_t* new_file = init_inode(disk, free_block, dest_directory->header.file_id, dest_image);

  uint8_t* file_data = NULL;
  size_t file_size = 0;
  TRACE("Reading file 0x%lx (%*s)", src_file->header.file_id, src_file->desc.name_length, src_file->desc.name);
  if (ccos_read_file(disk, src_file, src_image, &file_data, &file_size) == -1) {
  ...
  if (ccos_write_file(disk, new_file, dest_image, dest_image_size, file_data, file_size) == -1) {
    fprintf(stderr, "Unable to write file to file with id 0x%x!\n", free_block);
    free(file_data);
    return -1;
  }

  int res = add_file_to_directory(disk, dest_directory, new_file, dest_image, dest_image_size);
  free(file_data);
...
int ccos_delete_file(ccos_disk_t* disk, uint8_t* image_data, size_t data_size, ccos_inode_t* file) {
  if (ccos_is_dir(file)) {
    TRACE("Recursively deleting files in the directory %*s (0x%x)", file->desc.name_length, file->desc.name,
          file->header.file_id);
    uint16_t files = 0;
    ccos_inode_t** content = NULL;
    ccos_get_dir_contents(disk, file, image_data, &files, &content);
    for (int c = 0; c < files; c++) {
      ccos_delete_file(disk, image_data, data_size, content[c]);
    }
    free(content);
  }

  ccos_bitmask_list_t bitmask_list = find_bitmask_blocks(disk, image_data, data_size);
  if (bitmask_list.length == 0) {
    fprintf(stderr, "Unable to delete file: Unable to find image bitmask!\n");
    return -1;
  }

  if (delete_file_from_parent_dir(disk, file, image_data, data_size) == -1){
  ...
  for (int j = 0; j < blocks_count; ++j) {
    erase_block(disk, blocks[j], image_data, &bitmask_list);
  }
  free(blocks);

  while (file->content_inode_info.block_next != CCOS_INVALID_BLOCK) {
    if (remove_content_inode(disk, file, image_data, &bitmask_list) == -1) {
  ...
  erase_block(disk, file->header.file_id, image_data, &bitmask_list);
...
ccos_inode_t* ccos_add_file(ccos_disk_t* disk, ccos_inode_t* dest_directory, uint8_t* file_data, size_t file_size, const char* file_name,
  ...
  ccos_inode_t* new_file = init_inode(disk, free_block, dest_directory->header.file_id, image_data);
  ...
  TRACE("Writing file 0x%lx", new_file->header.file_id);
  if (ccos_write_file(disk, new_file, image_data, image_size, file_data, file_size) == -1) {
    fprintf(stderr, "Unable to write file to file with id 0x%x!\n", new_file->header.file_id);
    return NULL;
  }

  int res = add_file_to_directory(disk, dest_directory, new_file, image_data, image_size);
...
ccos_inode_t* ccos_get_root_dir(ccos_disk_t* disk, uint8_t* data, size_t data_size) {
  uint16_t superblock = 0;

  if (get_superblock(disk, data, data_size, &superblock) == -1) {
    fprintf(stderr, "Unable to get root directory: unable to get superblock!\n");
    return NULL;
  }

  return get_inode(disk, superblock, data);
}
...
size_t ccos_calc_free_space(ccos_disk_t* disk, uint8_t* data, size_t data_size) {
  uint16_t superblock = 0;
  if (get_superblock(disk, data, data_size, &superblock) == -1) {
  ...
  ccos_bitmask_list_t bitmask_list = find_bitmask_blocks(disk, data, data_size);
  if (bitmask_list.length == 0) {
    fprintf(stderr, "Unable to calculate free space on the image: Unable to find bitmask!\n");
    return -1;
  }

  if (get_free_blocks(disk, &bitmask_list, data_size, &free_blocks_count, &free_blocks) == -1) {
...
ccos_inode_t* ccos_get_parent_dir(ccos_disk_t* disk, ccos_inode_t* file, uint8_t* data) {
  uint16_t parent_dir_id = file->desc.dir_file_id;
  return get_inode(disk, parent_dir_id, data);
}
...
ccos_inode_t* ccos_create_dir(ccos_disk_t* disk, ccos_inode_t* parent_dir, const char* directory_name,
  ...
  ccos_inode_t* new_directory = ccos_add_file(disk, parent_dir, &directory_contents, get_dir_default_size(disk), filename, image_data, image_size);
  free(filename);
...
int ccos_rename_file(ccos_disk_t* disk, uint8_t* image_data, size_t image_size, ccos_inode_t* file,
  ...
  if (!is_root_dir(file)) {
    ...
    ccos_inode_t* parent_dir = ccos_get_parent_dir(disk, file, image_data);

    if (delete_file_from_parent_dir(disk, file, image_data, image_size) == -1){
    ...
    if (add_file_to_directory(disk, parent_dir, file, image_data, image_size) == -1){
