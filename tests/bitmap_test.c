...
static size_t copy_bitmap_bytes(ccos_disk_t* disk, uint8_t** snapshot) {
  ...
  *snapshot = malloc(snapshot_size);
  ...
  for (size_t i = 0; i < bitmask_list.length; ++i) {
    memcpy(*snapshot + i * bitmask_size, get_bitmask_bytes(bitmask_list.bitmask_blocks[i]), bitmask_size);
  }
...
static void clear_first_tail_bit(ccos_disk_t* disk) {
  ...
  size_t blocks_per_bitmask = get_bitmask_blocks(disk);
  ...
  update_bitmask_checksum(disk, bitmask_list.bitmask_blocks[bitmask_index]);
}
...
static void assert_free_blocks_count_is_valid(ccos_disk_t* disk) {
  ...
  cr_assert_eq(get_free_blocks_count(disk, &bitmask_list, &free_blocks_count), CCOS_OK);

  size_t free_space = 0;
  cr_assert_eq(ccos_calc_free_space(disk, &free_space), CCOS_OK);
  cr_assert_eq(free_space, free_blocks_count * get_block_size(disk));

  size_t block_count = disk->size / get_block_size(disk);

  if (free_blocks_count > 0) {
    uint16_t free_block = get_free_block(disk, &bitmask_list);
    cr_assert_lt(free_block, block_count);
  } else {
    cr_assert_eq(get_free_block(disk, &bitmask_list), CCOS_INVALID_BLOCK);
  }
}
...
static void mark_all_real_blocks_used(ccos_disk_t* disk) {
  ...
  while (free_block != CCOS_INVALID_BLOCK) {
    ...
    free_block = get_free_block(disk, &bitmask_list);
  }
}
...
static void assert_fresh_image(disk_format_t format, size_t disk_size, uint16_t expected_sector_size) {
  ...
  cr_assert_eq((disk.size / get_block_size(&disk)) % 8, 0);
...
Test(bitmap, reject_bad_allocated) {
  ...
  update_bitmask_checksum(&disk, bitmask_list.bitmask_blocks[0]);

  size_t free_blocks_count = 123;
  cr_assert_eq(get_free_blocks_count(&disk, &bitmask_list, &free_blocks_count), CCOS_EINVAL);
...
Test(bitmap, count_uses_bits, .init = cr_redirect_stderr) {
  ...
  cr_assert_eq(get_free_blocks_count(&disk, &bitmask_list, &expected_free_count), CCOS_OK);

  bitmask_list.bitmask_blocks[0]->allocated = 0;
  update_bitmask_checksum(&disk, bitmask_list.bitmask_blocks[0]);

  cr_assert_lt(expected_free_count, disk.size / get_block_size(&disk));
...
Test(bitmap, validate_allocated_count, .init = cr_redirect_stderr) {
  ...
  size_t free_count = 0;
  cr_assert_eq(get_free_blocks_count(&disk, &bitmask_list, &free_count), CCOS_OK);
  ...
  cr_assert_not(validate_disk_bitmap(&disk));
...
Test(bitmap, delete_large_file) {
  ...
  uint16_t* data_blocks = NULL;
  cr_assert_eq(get_file_blocks(&disk, file, &data_blocks_count, &data_blocks), CCOS_OK);
  cr_assert_gt(data_blocks_count, get_inode_max_blocks(&disk));
