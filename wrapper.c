...
typedef traverse_callback_result_t (*on_file_t)(ccos_disk_t* disk, ccos_inode_t* file, const uint8_t* data, const char* dirname, int level, void* arg);

typedef traverse_callback_result_t (*on_dir_t)(ccos_disk_t* disk, ccos_inode_t* dir, const uint8_t* data, const char* dirname, int level, void* arg);
...
static int traverse_ccos_image(ccos_disk_t* disk, ccos_inode_t* dir, uint8_t* data, const char* dirname, int level,
                               on_file_t on_file, on_dir_t on_dir, void* arg) {
  uint16_t files_count = 0;
  ccos_inode_t** dir_contents = NULL;
  if (ccos_get_dir_contents(disk, dir, data, &files_count, &dir_contents) == -1) {
  ...
  for (int i = 0; i < files_count; ++i) {
    ...
    if (ccos_is_dir(dir_contents[i])) {
      ...
      if (on_dir != NULL) {
        ...
        if ((res = on_dir(disk, dir_contents[i], data, dirname, level, arg)) != RESULT_OK) {
      ...
      int res = traverse_ccos_image(disk, dir_contents[i], data, subdir, level + 1, on_file, on_dir, arg);
      free(subdir);
      ...
      if (on_file != NULL) {
        traverse_callback_result_t res;
        if ((res = on_file(disk, dir_contents[i], data, dirname, level, arg)) != RESULT_OK) {
...
static traverse_callback_result_t print_file_info(
  ccos_disk_t* disk, ccos_inode_t* file, UNUSED const uint8_t* data,
...
int print_image_info(ccos_disk_t* disk, const char* path, uint8_t* data, size_t data_size, int short_format) {
  ccos_inode_t* root_dir = ccos_get_root_dir(disk, data, data_size);
  if (root_dir == NULL) {
...
static traverse_callback_result_t dump_dir_tree_on_file(
  ccos_disk_t* disk, ccos_inode_t* file, const uint8_t* data, const char* dirname,
  ...
  if (ccos_read_file(disk, file, data, &file_data, &file_size) == -1) {
...
static traverse_callback_result_t dump_dir_tree_on_dir(
  ccos_disk_t* disk, ccos_inode_t* dir, UNUSED const uint8_t* data,
...
int dump_image(ccos_disk_t* disk, const char* path, uint8_t* data, size_t data_size) {
  ccos_inode_t* root_dir = ccos_get_root_dir(disk, data, data_size);
  if (root_dir == NULL) {
  ...
  return dump_dir(disk, path, root_dir, data);
}
...
int dump_file(ccos_disk_t* disk, const char* path_to_dir, ccos_inode_t* file, uint8_t* image_data) {
  ...
  traverse_callback_result_t res = dump_dir_tree_on_file(disk, file, image_data, path_to_dir, 0, NULL);
...
int dump_dir(ccos_disk_t* disk, const char* path, ccos_inode_t* dir, uint8_t* data) {
    char* name_trimmed;
    if (dir == ccos_get_parent_dir(disk, dir, data)) {
  ...
  int res = traverse_ccos_image(disk, dir, data, dirname, 0, dump_dir_tree_on_file, dump_dir_tree_on_dir, NULL);
  free(dirname);
...
int dump_image_to(ccos_disk_t* disk, const char* path, uint8_t* data, size_t data_size, const char* destpath) {
  ...
  return dump_dir_to(disk, path, root_dir, data, destpath);
}
...
int dump_dir_to(ccos_disk_t* disk, const char* path, ccos_inode_t* dir, uint8_t* data, const char* destpath) {
  ...
  int res = traverse_ccos_image(disk, dir, data, dest, 0, dump_dir_tree_on_file, dump_dir_tree_on_dir, NULL);
  free(dest);
  TRACE("Image dump complete!");
  return res;
}
...
static int find_filename(ccos_disk_t* disk, ccos_inode_t* root_dir, uint8_t* data, const char* filename, ccos_inode_t** file,
                         int verbose) {
  find_file_data_t find_file_data = {.target_name = filename, .target_file = 0};

  if (traverse_ccos_image(disk, root_dir, data, "", 0, find_file_on_file, find_file_on_file, &find_file_data) == -1) {
...
int replace_file(ccos_disk_t* disk, const char* path, const char* filename, const char* target_name, uint8_t* data, size_t data_size,
                 int in_place) {
  const char* basename;
  ...
  if (find_filename(disk, root_dir, data, basename, &found_file, 1) != 0) {
  ...
  if (ccos_replace_file(disk, found_file, file_contents, file_size, data) == -1) {
...
static int do_copy_file(ccos_disk_t* disk, uint8_t* dest_data, size_t dest_size, ccos_inode_t* dest_root_dir, uint8_t* source_data,
                        ccos_inode_t* source_root_dir, const char* filename) {
  ccos_inode_t* source_file = NULL;

  if (find_filename(disk, source_root_dir, source_data, filename, &source_file, 1) != 0) {
    fprintf(stderr, "Unable to find file %s in the image!\n", filename);
    return -1;
  }

  const ccos_inode_t* source_parent_dir = ccos_get_parent_dir(disk, source_file, source_data);
  char* source_dir_name = short_string_to_string(ccos_get_file_name(source_parent_dir));

  ccos_inode_t* dest_directory = NULL;

  if (find_filename(disk, dest_root_dir, dest_data, source_dir_name, &dest_directory, 1) == -1) {
    fprintf(stderr, "Warn: Unable to find directory %s in dest image, will copy to the " PROGRAMS_DIR_1 " instead.\n",
            source_dir_name);
    if (find_filename(disk, dest_root_dir, dest_data, PROGRAMS_DIR_1, &dest_directory, 0) == -1 &&
        find_filename(disk, dest_root_dir, dest_data, PROGRAMS_DIR_2, &dest_directory, 0) == -1) {
  ...
  return ccos_copy_file(disk, dest_data, dest_size, dest_directory, source_data, source_file);
}

int copy_file(ccos_disk_t* disk, const char* target_image, const char* filename, uint8_t* source_data, size_t source_size, int in_place) {
  ...
  if ((root_dir = ccos_get_root_dir(disk, source_data, source_size)) == NULL) {
  ...
  if ((dest_root_dir = (ccos_inode_t*)ccos_get_root_dir(disk, dest_data, dest_size)) == NULL) {
    fprintf(stderr, "Unable to get root directory of the target image!\n");
    free(dest_data);
    return -1;
  }

  if (do_copy_file(disk, dest_data, dest_size, dest_root_dir, source_data, root_dir, filename) == -1) {
...
int add_file(ccos_disk_t* disk, const char* image_path, const char* file_path, const char* file_name,
  ...
  if (find_filename(disk, root_dir, data, PROGRAMS_DIR_1, &dest_dir, 0) == -1 &&
      find_filename(disk, root_dir, data, PROGRAMS_DIR_2, &dest_dir, 0) == -1) {
    fprintf(stderr, "Warn: Unable to find directory %s in dest image, will add file to the root directory instead\n",
            PROGRAMS_DIR_1);
    dest_dir = root_dir;
  }

  ccos_inode_t* new_file = ccos_add_file(disk, dest_dir, file_data, file_size, file_name, data, data_size);
  free(file_data);
  if (new_file == NULL) {
...
int delete_file(ccos_disk_t* disk, const char* path, const char* filename, int in_place) {
  ...
  ccos_inode_t* root_dir = ccos_get_root_dir(disk, data, size);
  ccos_inode_t* file = NULL;
  if (find_filename(disk, root_dir, data, filename, &file, 1) != 0) {
    fprintf(stderr, "Unable to find file %s in the image!\n", filename);
    free(data);
    return -1;
  }

  if (ccos_delete_file(disk, data, size, file) == -1) {
...
int create_directory(ccos_disk_t* disk, char* path, char* directory_name, uint8_t* image_data, size_t image_size, int in_place) {
  if (path == NULL) {
    fprintf(stderr, "No target image is provided to copy file to!\n");
    return -1;
  }
  ...
  ccos_inode_t* root_dir = ccos_get_root_dir(disk, image_data, image_size);
  if (root_dir == NULL) {
    fprintf(stderr, "Unable to add file to image: Unable to get root directory!\n");
    return -1;
  }

  if (ccos_create_dir(disk, root_dir, directory_name, image_data, image_size) == NULL) {
...
int rename_file(ccos_disk_t* disk, char* path, char* file_name, char* new_name, uint8_t* image_data, size_t image_size, int in_place) {
  if (path == NULL) {
    fprintf(stderr, "No target image is provided to copy file to!\n");
    return -1;
  }
  ...
  ccos_inode_t* file = NULL;
  if (find_filename(disk, root_dir, image_data, file_name, &file, 1) != 0) {
    fprintf(stderr, "Unable to find file %s in the image!\n", file_name);
    return -1;
  }

  if (ccos_rename_file(disk, image_data, image_size, file, new_name, NULL) == -1) {
